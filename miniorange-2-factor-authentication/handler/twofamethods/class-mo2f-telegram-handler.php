...
if ( ! class_exists( 'Mo2f_TELEGRAM_Handler' ) ) {
	...
	class Mo2f_TELEGRAM_Handler {
		 ...
		 * @return void
		 */
		public function mo2f_send_otp( $telegram_id, $session_id_encrypt, $user_details, $message ) {
			$mocurl      = new MocURL();
		 ...
		 * @return void
		 */
		public function mo2f_process_inline_send_otp( $content, $user_details, $session_id_encrypt, $message ) {
		 ...
		 * @return void
		 */
		public function mo2f_validate_otp( $otp_token, $session_id_encrypt, $user_details, $prev_chat_id, $post ) {
			global $mo2fdb_queries;
			$current_chat_id = get_transient( $session_id_encrypt . 'mo2f_temp_chatID' );
			...
			$mocurl              = new MocURL();
			$content             = $mocurl->mo2f_validate_telegram_code( $otp_token, $mo2f_transaction_id, $session_id_encrypt );
			$this->mo2f_process_inline_validate_otp( $content, $user_details, $session_id_encrypt );
		}
		...
		public function mo2f_process_inline_validate_otp( $content, $user_details, $session_id_encrypt ) {
			if ( 'ERROR' === $content['status'] ) {
				...
				update_user_meta( $user->ID, 'mo2f_chat_id', get_transient( $session_id_encrypt . 'mo2f_temp_chatID' ) );
				delete_transient( $session_id_encrypt . 'mo2f_temp_chatID' );
				delete_transient( $session_id_encrypt . 'mo2f_otp_token' );
				delete_transient( $session_id_encrypt . 'mo2f_telegram_time' );
				delete_transient( $session_id_encrypt . 'mo2f_otp_send_true' );
				$common_helper = new Mo2f_Common_Helper();
